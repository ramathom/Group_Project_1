...
%checks that the coordinates at each frame are complete (does not contain NaN
...
for frame = 1:num_of_frames(1)
    if ~isnan(marker_of_interest(frame,1)) && ~isnan(marker_of_interest(frame,2)) && ~isnan(marker_of_interest(frame,3))
        time_windows = [time_windows frame]; %updating matrix to include each subsequent frame that's complete
    end
end
end